...
class Plot(QtGui.QWidget):
    ...
    def singlePlot(self, address, well):
        ...
        if mode == 0:
            ...
            self.plot_widget.axis.set_xlim([max(len(line)-12,0), len(line)+1])
            ...
            self.plot_widget.axis.plot([max(scan-12,1)-2, max(scan-5,0)],\
            [slope*(max(scan-12,1))+intercept,slope*max(scan-3,0)+intercept],'--')
            self.plot_widget.axis.plot([max(scan-12,1)-2, max(scan-2,0)],\
            [slope*(max(scan-12,1))+intercept+stderr*sigma,slope*(scan)+intercept+stderr*sigma],'--')

            self.plot_widget.axis.plot(Data.ix[address,well].values,'-o')
            try:
                self.plot_widget.axis.plot([scan-3,scan-2,scan-1],\
            ...
            self.plot_widget.label.setText('%d_%d : # %d'%(address, well, 1+int(position[address-1][-3:])))
...
def main():
    ...
    Roi_Data_file = 'C:/Users/YTanaka/Desktop/20170131/Roi_Data.csv'
    position_file = 'Z:/RealTimeExport/20170131/20170131_real_2.xml'
    nd2_file = 'Z:/images/20170131/NDSequence'
    time_file = 'C:/Users/YTanaka/Desktop/20170131/time.csv'
    sample_file = 'C:/Users/YTanaka/Desktop/20170131/sample.csv'
    status_file = 'C:/Users/YTanaka/Desktop/20170131/status.csv'
    """
