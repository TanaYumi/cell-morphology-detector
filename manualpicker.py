# -*- coding: utf-8 -*-
"""
Created on Wed Sep 28 19:55:56 2016
...
class Run(QtGui.QWidget):
    ...
    def analyze(self):
        ...
        global pic
        global scan
        rawData, rawTime, pic = Analyze.Analyze(scan, protein, nd2_file, Roi_Data_file, status_file, time_file, n)
    ...
    def read(self):
        ...
        suggestion = readFile_v3.linear_fit_suggest(scan, Data, max(scan-10,0))#readFile_v2.fitting(scan, Data, timelag, threthold)
        try:
            for i in range(n):
                for j in range(4):
                    Grid[i].Button[j].setStyleSheet('QPushButton {background-color: %s }'%color[Status.ix[i+1,j+1]['Status']].name())
            for i in suggestion:
                if Status.ix[i[0],i[1]]['Status'] == 0:
                    body = body + '%d_%d,\n'
                    Grid[i[0]-1].Button[i[1]-1].setStyleSheet('QPushButton {background-color: #ff0000 }')
                #sendGmail.sendGmail(to, sub, body)
        except:
            print 'miss'
...
class Plot(QtGui.QWidget):
    ...
    def singlePlot(self, address, well):
        ...
        if mode == 0:
            ...
            self.plot_widget.axis.plot(Data.ix[address,well].values)
...
class Address(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        for i in range(4):
            ...
            ('QPushButton {background-color: %s }'%color[Status.ix[self.address,i+1]['Status']].name())
...
def main():
    ...
    """
    Roi_Data_file = '/Volumes/SINGLECELL/sampledata/test.csv'
    position_file = '/Volumes/SINGLECELL/sampledata/160418.xml'
    nd2_file = '/Volumes/SINGLECELL/sampledata/20160425_IL33andIL2_qPCR'
    time_file = '/Volumes/SINGLECELL/sampledata/time.csv'
    sample_file = '/Volumes/SINGLECELL/sampledata/sample.csv'
    status_file = '/Volumes/SINGLECELL/sampledata/status.csv'
    
    n = 159#変更する
    protein = 2#フィルター
    scan = 1#新しくGUIを立ち上げる時は変更
    pos_n = 996
    
    to = 'gomakanabun@gmail.com'
    sub = 'サンプルを回収せよ'
    body = '分泌が疑われるサンプル：\n'
